...
def run_experiment(config, param, tau, res, l_scale, d_scale, n_scale, graph_file, mean_range=None, stall_prop=None):
    ...
    # 寫入 summary
    summary_path = './testvar_result0616/resultvar0618_summary.csv'
    header = [
...
def main():
    ...
    # config 2 (pSA) sweep
    l_scale_values_2 = [0.0, 0.25, 0.5]
    d_scale_values_2 = [0.0, 0.1, 0.2]
    n_scale_values_2 = [1.0, 1.5, 2.0]
    print("Testing config 2 (pSA)...")
    comb2 = list(product(params, taus, res_values, l_scale_values_2, d_scale_values_2, n_scale_values_2))
    for graph_file in graph_files:
        for param, tau, res, l_scale, d_scale, n_scale in tqdm(comb2, desc=f"pSA-{graph_file}"):
            mean_ratio, max_ratio = run_experiment(2, param, tau, res, l_scale, d_scale, n_scale, graph_file=graph_file)
            print(f"[pSA] {graph_file} param={param}, tau={tau}, res={res}, l_scale={l_scale}, d_scale={d_scale}, n_scale={n_scale} | mean_ratio={mean_ratio}, max_ratio={max_ratio}")
