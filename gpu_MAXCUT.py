...
def parse_arguments():
    ...
    parser.add_argument('--cpu', action='store_true', help='Run in CPU-only mode (no CUDA)')
    return parser.parse_args()

def initialize_cuda(args):
    if args.cpu:
        print("CPU-only mode: skipping CUDA initialization")
        return None
    if not HAVE_CUDA:
        raise RuntimeError("CUDA/PyCUDA 未可用，且未指定 --cpu。請安裝 CUDA 或改用 --cpu 模式。")
...
def cpu_run_trials(stall_prop, mean_range, file_base, config, vertex, min_cycle, trial, tau, Mshot, h_vector, G_matrix, J_matrix, I0_min, I0_max, beta):
    # 僅支援最基本 pSA（config == 2）的 CPU 版本
    if config != 2:
        raise NotImplementedError("CPU-only 模式目前僅支援 --config 2 (pSA)")

    def energy_delta_for_flip(i, spin_vec):
        # ΔE 對應 energy_calculate 的定義
        local_field = h_vector[i, 0] + np.dot(J_matrix[i, :], spin_vec[:, 0])
        return 2.0 * spin_vec[i, 0] * local_field
    ...
    for k in range(trial + 1):
        ...
        while I0 <= I0_max:
            for _ in range(tau):
                # 逐點 Metropolis 更新（隨機順序）
                indices = np.random.permutation(vertex)
                for i in indices:
                    dE = energy_delta_for_flip(i, spin_vector)
                    if dE <= 0:
                        spin_vector[i, 0] *= -1
                    else:
                        # I0 當作溫度比例參數，避免 0 除
        ...
        annealing_time_ms = (t1 - t0) * 1000.0

        cut_val = cut_calculate(vertex, -J_matrix, spin_vector)  # G_matrix = -J for cut
        min_energy = energy_calculate(vertex, h_vector, J_matrix, spin_vector)

        cut_list.append(int(cut_val))
        time_list.append(annealing_time_ms)
        energy_list.append(min_energy)

        print('Graph:', file_base)
        print('Time:', annealing_time_ms)
...
def main():
    ...
    try:
        if args.cpu or not HAVE_CUDA:
            cut_list, time_list, energy_list = cpu_run_trials(args.stall_prop, args.mean_range, file_base, config, vertex, min_cycle, trial, tau, Mshot, h_vector, G_matrix, J_matrix, I0_min, I0_max, beta)
        else:
