...
def sigma_to_bitmask_k(sigma, sigma_start, sigma_end, n):
    ...
    映射策略：線性映射
    - sigma = sigma_start → k = n（最大隨機性）
    - sigma = sigma_end   → k = 0（無隨機性）
    ...
    返回:
        k: bitmask 中 1 的數量 (0 <= k <= n)
    """
    if sigma_start == sigma_end:
        return n // 2
    
    # 線性映射: sigma_start → k=n, sigma_end → k=0
    k = int(round(n * (sigma - sigma_end) / (sigma_start - sigma_end)))
    k = max(0, min(n, k))  # 確保在 [0, n] 範圍內
...
def probit_fitting_hardware_synchronous(J_matrix, timesteps, sigma_start, sigma_end, schedule='linear', record_energy=False, epsilon=0.1):
    ...
    print(f'\n[Bitmask Generator 監控] - 每個 timestep 都輸出完整 bitmask')
    print(f'  n (spin 數量) = {N}')
    print(f'  k 範圍: {N} (高溫) → 0 (低溫)')
    print(f'  理論 σ_noise 範圍: √({N}/2)={np.sqrt(N/2):.2f} → 0')
    ...
    for t in range(timesteps):
        ...
        #   - 2N 個 MUX: 前 N 個 (0→0, 1→TRNG)，後 N 個 (0→1, 1→TRNG)
        #   - 2N 個類比 TRNG: 產生 {0, 1}
        #   - All-1 Xbar: 將 2N 個 MUX 輸出加總
        #   - 數位修正: noise = total - N (使均值為 0)
        #
        # 溫度映射: sigma_start → k=N (高溫), sigma_end → k=0 (低溫)
        # 硬體雜訊標準差: σ_hw = √(k/2)
