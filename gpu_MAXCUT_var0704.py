...
def run_trials(args, file_base, config, vertex, min_cycle, trial, tau, Mshot, gpu_code, h_vector, G_matrix, J_matrix, sigma_vector, nrnd_vector, I0_min, I0_max, beta, max_cycle, threads_per_block, blocks_per_grid):
    ...
    for k in range(trial + 1):
        ...
        last_spin_vector = np.empty_like(spin_vector)  # 用於記錄 energy 的 spin_vector
    ...
    plt.tight_layout()
    # 建立資料夾並加上時間戳記和參數到檔名
    os.makedirs('./origin_energy_plots', exist_ok=True)
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # 儲存為 PNG
    png_filename = f'./origin_energy_plots/energy_vs_cycles_{file_base}_config{config}_tau{tau}_res{res}_l{l_scale}_d{d_scale}_n{n_scale}_{timestamp}.png'
    plt.savefig(png_filename)
    
    # 儲存為 Excel
    excel_filename = f'./origin_energy_plots/energy_vs_cycles_{file_base}_config{config}_tau{tau}_res{res}_l{l_scale}_d{d_scale}_n{n_scale}_{timestamp}.xlsx'
