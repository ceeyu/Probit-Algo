    ...
    __global__ void annealing_module(float stall_prop, int vertex, float mem_I0, int8_t *h_vector, int8_t *J_matrix, int *spin_vector, float *rnd, float *lambda, float *delta, int *nu, int count_device)
    {
        ...
        if (i < vertex)
        {
            ...
            if (i % 2 == 0) { // 偶數 index
                ...
                do_update = (rand_val < 0.5f);
            }
            // 奇數 index do_update 會是 true
            if (do_update && (count_device % nu[i] == 0))
            {
                D_res = h_vector[i];
                __syncthreads();
                for (k = 0; k < vertex; k++)
                ...
                // 亂數 stall_prop 只針對 do_update==true 的情況
                float rand_val2 = curand_uniform(&state);
                if (rand_val2 >= stall_prop)
                {
