...
def load_gpu_code(config):
    if config == 1:
        ...
        cu_file = 'tapsa_annealing_kernel_var0702.cu'
    elif config == 4:
        cu_file = 'spsa_annealing_kernel_var0702.cu'
...
def run_trials(args, file_base, config, vertex, min_cycle, trial, tau, Mshot, gpu_code, h_vector, G_matrix, J_matrix, sigma_vector, nrnd_vector, I0_min, I0_max, beta, max_cycle, threads_per_block, blocks_per_grid):
    ...
    cut_list = []
    energy_list = []
    energy_per_cycle_last_trial = [] # 只記錄最後一個 trial 的能量
    for k in range(trial + 1):
        print("######## Trial", k + 1, " ###########")
        spin_vector = (np.random.randint(0, 2, (vertex, 1)) * 2 - 1).astype(np.int32)
        last_spin_vector = np.empty_like(spin_vector)  # <-- 移到這裡，確保 spin_vector 已宣告
        ...
        mean_range_max = 10
        while I0 <= I0_max:
            # 動態調整 mean_range，隨 I0 由小到大
            I0_log = np.log10(I0)
            # 線性插值
            mean_range = int(round(mean_range_min + (mean_range_max - mean_range_min) * (I0_log - I0_min_log) / (I0_max_log - I0_min_log)))
            mean_range = max(mean_range_min, min(mean_range, mean_range_max))
            ...
            for i in range(tau):
                ...
                if k == trial:
                    ...
                    energy = energy_calculate(vertex, h_vector, J_matrix, last_spin_vector)
        ...
        # 取得最後一次的 spin_vector 計算最終 energy
        cuda.memcpy_dtoh(last_spin_vector, spin_vector_gpu)
...
def save_results(best_known, trial, cut_list, time_list, energy_list, args, file_base, first_line, second_line, third_line, fourth_line,  sigma_vector, I0_min, I0_max, total_time):
    ...
    csv_file_name1 = f'./testvar_result0702/{alg}_result_unique{args.unique}_config{args.config}_cycle{args.cycle}_trial{args.trial}_tau{args.tau}_thread{args.thread}_param{args.param}_res{args.res}.csv'
    csv_file_name2 = f'./testvar_result0702/{alg}_cut_unique{args.unique}_config{args.config}_cycle{args.cycle}_trial{args.trial}_tau{args.tau}_thread{args.thread}_param{args.param}_res{args.res}.csv'
